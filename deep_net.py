...
class FashionNet():
    ...
    def build(self, train_data): # VGG16 architecture
        ...
        x = layers.MaxPool2D()(x)
        x = layers.Flatten()(x)
        cat_net = layers.Dense(4, activation="softmax")(x)
        col_net = layers.Dense(3, activation="softmax")(x)
        ...
        H = self.net.fit(x=self.trainX, y=train_data, batch_size=self.batch_size,
                     epochs=self.epochs, verbose=1, validation_data=(self.testX, train_data))
        return self.net, H
